...
int test_isxdigit() {
  ...
  unsigned char non_xdigit[] = "\x00\x01\x02\x03\x04\x05\x06\a\b\t\n\v\f\r" \
    "\x0E\x0F\x10\x11\x12\x13\x14\x15\x16\x17\x18\x19\x1A\x1B\x1C\x1D\x1E\x1F" \
    " !\"#$%&'()*+,-./:;<=>?@GHIJKLMNOPQRSTUVWXYZ[\\]^_`ghijklmnopqrstuvwxyz{" \
    "|}~\x7F";

  for (unsigned int i=0; i < sizeof(xdigit)-1; i++) {
    assert(isxdigit(xdigit[i]) == true);
  }

  for (unsigned int i=0; i < sizeof(non_xdigit)-1; i++) {
    assert(isxdigit(non_xdigit[i]) == false);
  }
